...
export default function App() {
  ...
  if (page === "tool") {
    return <Delphi paymentStatus={paymentStatus} startCheckout={startCheckout} onHome={() => setPage("home")} />;
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: FF }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "18px 56px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(250,247,242,0.95)", backdropFilter: "blur(10px)", borderBottom: "1px solid " + C.border }}>
        ...
        <button onClick={() => setPage("tool")} style={{ background: C.accent, color: C.white, border: "none", borderRadius: 3, padding: "11px 28px", fontSize: 13, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", cursor: "pointer", fontFamily: FF }}>Start Evaluation</button>
      </nav>
      ...
      <div style={{ minHeight: "86vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "140px 48px 80px", maxWidth: 860, margin: "0 auto", animation: "fadeUp 0.65s ease" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 24, flexWrap: "wrap" }}>
          <button onClick={() => setPage("tool")} style={{ background: C.accent, color: C.white, border: "none", borderRadius: 3, padding: "16px 40px", fontSize: 14, fontWeight: 700, letterSpacing: 2.5, textTransform: "uppercase", cursor: "pointer", fontFamily: FF }}>Start your evaluation</button>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
          {[
            ...
            <div key={idx} style={{ background: card.featured ? C.accent : C.white, border: "1.5px solid " + (card.featured ? C.accent : C.border), borderRadius: 10, padding: "44px 40px", display: "flex", flexDirection: "column" }}>
              ...
              <button onClick={() => setPage("tool")} style={{ background: card.featured ? C.white : C.accent, color: card.featured ? C.accent : C.white, border: "none", borderRadius: 40, padding: "12px 28px", fontSize: 13, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", fontFamily: FF, alignSelf: "flex-start", cursor: "pointer" }}>{card.btn}</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "88px 56px", background: C.bg, borderTop: "1px solid " + C.border }}>
        <div style={{ maxWidth: 1060, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20, maxWidth: 860 }}>
            {[
              { type: "fractional", label: "Fractional Stack Report", price: "$30", period: "", desc: "For fractional executives building their own practice stack.", btn: "Get Report", featured: false },
              { type: "single_report", label: "One Report", price: "$175", period: "", desc: "Full evaluation, one category. Most organizations evaluate more than once.", btn: "Start Evaluation", featured: false },
              { type: "annual", label: "Annual Subscription", price: "$300", period: "/yr", desc: "Unlimited evaluations. Both report types. Cancel anytime.", btn: "Subscribe", featured: true },
            ].map((card, i) => (
              <div key={i} style={{ background: card.featured ? C.accent : C.white, border: "1.5px solid " + (card.featured ? C.accent : C.border), borderRadius: 8, padding: "32px", display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                <button onClick={() => startCheckout(card.type)} style={{ background: card.featured ? C.white : C.accent, color: card.featured ? C.accent : C.white, border: "none", borderRadius: 3, padding: "12px 20px", fontSize: 12, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", fontFamily: FF, cursor: "pointer", width: "100%" }}>{card.btn}</button>
              </div>
            ))}
          </div>
          <p style={{ fontSize: 13, fontWeight: 500, color: C.textLight, marginTop: 16 }}>Limited access available. No credit card required to start.</p>
        </div>
      </div>
      ...
      <div style={{ textAlign: "center", padding: "88px 48px", background: C.dark }}>
        ...
        <button onClick={() => setPage("tool")} style={{ background: C.white, color: C.accent, border: "none", borderRadius: 3, padding: "16px 40px", fontSize: 14, fontWeight: 700, letterSpacing: 2.5, textTransform: "uppercase", cursor: "pointer", fontFamily: FF }}>Start your evaluation</button>
      </div>
