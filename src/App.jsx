...
export default function App() {
  ...
  const SignInModal = () => (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}
      onClick={e => { if (e.target === e.currentTarget) { setShowSignIn(false); setSignInStatus("idle"); setSignInEmail(""); } }}>
      <div style={{ background: C.bg, borderRadius: 8, padding: "40px 36px", maxWidth: 420, width: "100%", animation: "fadeUp 0.3s ease" }}>
        ...
        {signInStatus === "sent" ? (
          ...
          <>
            ...
            {signInError && <p style={{ fontSize: 13, color: "#C0392B", marginBottom: 10 }}>{signInError}</p>}
            <button onClick={handleSignIn} disabled={!signInEmail.trim() || signInStatus === "sending"}
              style={{ background: !signInEmail.trim() ? C.border : C.accent, color: C.white, border: "none", borderRadius: 4, padding: "12px 24px", fontSize: 13, fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", fontFamily: FF, cursor: "pointer", width: "100%" }}>
    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: FF }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, padding: "18px 56px", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(250,247,242,0.95)", backdropFilter: "blur(10px)", borderBottom: "1px solid " + C.border }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
          ...
          {user ? (
            <>
              <button onClick={() => setPage("tool")} className="nav-link" style={{ background: "none", border: "none", fontSize: 14, color: C.textLight, fontWeight: 500, cursor: "pointer", fontFamily: FF, transition: "color 0.15s" }}>My Reports</button>
            ...
            <button onClick={() => setShowSignIn(true)} className="nav-link" style={{ background: "none", border: "none", fontSize: 14, color: C.textLight, fontWeight: 500, cursor: "pointer", fontFamily: FF, transition: "color 0.15s" }}>Sign in</button>
          )}
